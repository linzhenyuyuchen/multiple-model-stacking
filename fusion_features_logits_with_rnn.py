...
#######################################################
feature_num = 4 # features from last linear layer of CNN models
models_num = 11 # multiple model with different backbone
feature_dim = 128 # dimension of features
...
drop_out = 0.5 # hyper parameter
#######################################################
class SequenceModel(nn.Module):
    def __init__(self):
        super(SequenceModel, self).__init__()
        ratio = 4
        model_num = models_num + 1
        # seq model 1
...
if __name__ == "__main__":
    ...
    x = torch.rand((batch_size,models_num,seq_len,class_num))
    model = SequenceModel()
    out, out0 = model(fea,x)
    # logits from features
    print(out0.shape) # (batch_size,1,seq_len,class_num)
    # final logits from fusion of features and previous logits
    print(out.shape)  # (batch_size,1,seq_len,class_num)